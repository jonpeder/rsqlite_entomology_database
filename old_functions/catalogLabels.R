# catalogLabels, Jon Peder Lindemann, 14.06.2021
# Print insectlabels from database query
# CONN | SQLite database connection
# catalogNumber | a vector or character string, specifying catalog numbers of records to be printed
...
catalogLabels <- function(CONN, 
  ...
  for (i in 1:length(catalogNumber)) {
    query_tmp <- dbGetQuery(CONN, paste("SELECT
      ...
      WHERE Occurrences.catalogNumber = '", catalogNumber[i], "'", sep = ""))
  ...
  if (type == "type") {
    ...
    insectlabel::insectlabel(label_df = query, n = 1, x = 40, y = 8, filename = filename, family = font_family, text_order = list(c(3,4,5,6),c(7,8),c(10,11,12),c(13,17),16,2), 
                             QR_data = 3, font = c(1,1,1,1,1,2), fontsize = 3, linedist = 1, tx = 11, ty = -1, QRd = 1.6, QRx = 4.4, 
